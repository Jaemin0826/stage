...
const TopHeader = styled.div`
  ...
  align-items: center;
  z-index: 9;
  pointer-events: none; /* 헤더가 인터랙션을 막지 않도록 */
...
const BottomDock = styled.div`
  ...
  bottom: 0;
  z-index: 7;
  pointer-events: none; /* 3D 인터랙션을 막지 않음 */
  ...
  &::before {
    ...
    height: 10px;
    background: linear-gradient(180deg, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0.0) 60%);
    filter: blur(0.4px);
  }
`;
...
const DockInner = styled.div`
  ...
  justify-content: space-between;
  padding: clamp(16px, 3.2vw, 40px) clamp(16px, 4vw, 56px);
`;
...
const GradientTitle = styled.h3`
  ...
  font-weight: 800;
  letter-spacing: 0.01em;
  background: linear-gradient(90deg, #ffffff 0%, #ff8052 60%, #ff4b1c 100%);
...
function App() {
  ...
  return (
    <CanvasWrap>
        ...
        <Selection>
          ...
          <Hoverable
            ...
            onClick={(e) => {
              ...
              focusCamera([6, 3, -2], [2, 0, 2], 1.4);
            }}
          >
            <Model url="/Stage_Shell.glb" position={[0, 0, 0]} metallic />
          </Hoverable>
          ...
          <Hoverable
            ...
            onClick={(e) => {
              ...
              if (orbitDelayRef.current) orbitDelayRef.current.kill();
              setFollowChair(false);
              focusCamera([-3, 0, -2.8], [0, 0, 0], 1.4);
            }}
          >
            <Model url="/Stage_Wheel.glb" position={[0, 0, 0]} metallic />
          </Hoverable>
      ...
      {focusedId && focusContent[focusedId] && (
        <FocusPanel key={focusedId}>
          <FocusTitleRow>
            <TipIcon src="/Tip.svg" alt="tip" />
            <FocusTitle>{focusContent[focusedId].title}</FocusTitle>
          </FocusTitleRow>
          <FocusBody>
            {/* 줄바꿈 문자(\n)를 브라우저 줄바꿈으로 표현 */}
            {focusContent[focusedId].body.split("\n").map((line, i) => (
              <span key={i}>
                {line}
                <br />
              </span>
        ...
        <BottomDock>
          <DockInner>
            <DockLeft>
              ...
              <DockPara
                dangerouslySetInnerHTML={{ __html: dockContent[focusedId].body }}
              />
            </DockLeft>
            <DockRight>
              <ArrowImg src="/Arrow.svg" alt="arrow" />
            </DockRight>
          </DockInner>
        </BottomDock>
      )}
      {!introActive && (
        <TopHeader>
          <HeaderInner>
            <LeftGroup>
              <LogoSmall src="/Text.svg" alt="Logo" />
              <Sep>|</Sep>
      ...
      {introActive && (
        <IntroOverlay ref={overlayRef} onClick={handleStart}>
          <Logo src="/Logo.svg" alt="Logo" ref={logoRef} />
